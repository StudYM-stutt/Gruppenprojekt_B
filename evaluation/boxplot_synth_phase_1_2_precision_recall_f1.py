...
def create_metric_plot(input_file, metric_column, metric_label):
    """
    Create one boxplot per metric (Precision, Recall, F1-Score).
    Within each granularity group, all prompt variants are shown side by side.
    """
    df = read_csv_robustly(input_file)

    required_columns = {"prompt_name", metric_column}
    ...
    if df.empty:
        raise ValueError(
            f"{input_file.name}: No valid numeric {metric_label} values found."
        )
    ...
    if use_granularity:
        ...
        title = (
            f"{metric_label} der Promptvarianten nach Granularitätsstufe "
            f"({phase_label})"
        )
        ...
        title = f"{metric_label} der Promptvarianten"
    ...
    box_width = min(
        0.16,
        total_group_width / max(number_of_prompts, 1),
    )
    ...
    # Einheitliche Skala für Precision, Recall und F1:
    # 0 bis 1 in Schritten von 0,05.
    ax.set_ylim(0.0, 1.0)
    ...
    output_file = input_file.with_name(
        f"{input_file.stem}_{metric_column}_boxplot.png"
    )
    ...
    print(
        f"{input_file.name}: {metric_label}-Boxplot gespeichert unter: "
        f"{output_file}"
    )
...
def create_plot(input_file):
    """
    Create three boxplots for each input CSV:
    ...
    df_columns = read_csv_robustly(input_file).columns
    column_lookup = {
    ...
    for canonical, label in metrics:
        found = None

        for candidate in alternatives[canonical]:
            if candidate in column_lookup:
                found = column_lookup[candidate]
                break

        if found is None:
            raise ValueError(
                f"{input_file.name}: Keine Spalte für {label} gefunden."
            )

        resolved_metrics.append((found, label))

    for metric_column, metric_label in resolved_metrics:
        create_metric_plot(
            input_file,
            metric_column,
            metric_label,
